<template>
  ...
  <div v-else-if="!!currentPost.data && !currentPost.loading">
    ...
    <Form enctype="multipart/form-data" @submit="onSubmit">
      ...
      <creditor
        :content="currentPost.data.description"
        @inputListener="currentValue"
      />
      <Field
        name="description"
        type="text"
...
<script>
import creditor from "../../../../components/creditor-comp";
...
export default {
  ...
  methods: {
    ...mapActions(["GET_CURRENT_POST", "UPDATE_POST"]),
    onSubmit(data) {
      this.UPDATE_POST({
  ...
  computed: {
    ...mapGetters(["currentPost", "putPost"]),
  },
  mounted() {
